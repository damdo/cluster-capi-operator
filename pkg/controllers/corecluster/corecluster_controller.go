...
var (
	...
	errMappingOpenshiftPlatformToInfraClusterKindAndVersion = errors.New("failed to map Infrastructure PlatformType to a Cluster API infra cluster kind, platform not supported")
	errOpenshiftInfraShouldNotBeNil                         = errors.New("infrastructure object should not be nil")
	errOpenshiftInfrastructureNameShouldNotBeEmpty          = errors.New("infrastructure object's status.infrastructureName should not be empty")
)
...
func (r *CoreClusterReconciler) Reconcile(ctx context.Context, req reconcile.Request) (ctrl.Result, error) {
	...
	if err != nil {
		return ctrl.Result{}, fmt.Errorf("failed to obtain ocp infrastructure name: %w", err)
	}
...
func (r *CoreClusterReconciler) ensureCoreCluster(ctx context.Context, clusterObjectKey client.ObjectKey, logger logr.Logger) (*clusterv1.Cluster, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("unable to map Infrastucture resource platform type to Cluster API infra cluster kind: %w", err)
	}
...
func mapOCPPlatformToInfraClusterKindAndVersion(platform configv1.PlatformType) (string, string, error) {
	...
	default:
		return "", "", fmt.Errorf("%w: %q", errMappingOpenshiftPlatformToInfraClusterKindAndVersion, platform)
	}
}
